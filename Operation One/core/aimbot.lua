...
aimbot.init = function()
    ...
    local old_fire, old_invoke

    repeat task.wait() until Instance.FireServer
    old_fire = clonefunction(Instance.FireServer)

    repeat task.wait() until Instance.InvokeServer
    old_invoke = clonefunction(Instance.InvokeServer)
    ...
    hook_function(Instance.FireServer, function(self, ...)
        ...
        if #args >= 2 and typeof(args[1]) == "Vector3" and typeof(args[2]) == "Vector3" then
            local _, _, _, part = find_closest()
            if part then
                local origin = args[1]
                local noise = Vector3.new(math.random(-20,20), math.random(-20,20), math.random(-20,20)) / 1000
                args[2] = ((part.Position + settings.hitbox_offset + noise) - origin).Unit * math.random(8500, 11500)
            end
        end
